...
def _get_arcade_analog_device(system: Emulator) -> str:
    ...
    if selected_device == 'external_mouse':
        return 'external_mouse'
    if selected_device == 'left_stick':
        return 'left_stick'
    if selected_device == 'right_stick':
        return 'right_stick'

    # Backward compatibility with existing configs.
    if system.config.get_bool('use_mouse') or system.name in ['macintosh', 'archimedes']:
        return 'external_mouse'
    return 'right_stick'


def _arcade_analog_device_uses_mouse(system: Emulator) -> bool:
    return _get_arcade_analog_device(system) == 'external_mouse'
...
def _mame_options(
    ...
    # Select whether analog arcade controls come from a mouse-like device or the gamepad.
    _set(coreSettings, 'mame_mouse_enable', 'enabled' if _arcade_analog_device_uses_mouse(system) else 'disabled')
...
def _mame078plus_options(
    ...
    # Control Mapping
    default_analog_mode = 'analog' if _get_arcade_analog_device(system) in ('left_stick', 'right_stick') else 'digital'
    _set_from_system(coreSettings, 'mame2003-plus_analog', system, 'mame2003-plus_analog', default=default_analog_mode)
    ...
    if system.config.use_guns and guns:
        xy_device = 'lightgun'
    elif _arcade_analog_device_uses_mouse(system):
        xy_device = 'mouse'
    else:
        xy_device = 'disabled'
    _set(coreSettings, 'mame2003-plus_xy_device', xy_device)
